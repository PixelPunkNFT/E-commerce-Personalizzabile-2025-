...
export function login(email, password) {
  ...
  return async function (dispatch) {
    try {
      ...
      // Salva i dati dell'utente nel sessionStorage dopo il login
      if (data.user) {
...
export const load_UserProfile = () => async (dispatch) => {
  try {
    ...
    if (userData !== "undefined" && userData && userData !== undefined) {
      ...
      // Se non ci sono dati nel sessionStorage, l'utente non è autenticato
      dispatch({ type: LOAD_USER_FAIL, payload: "Utente non autenticato" });
    }
  } catch (error) {
...
export function updatePassword(userPassWord) {
  return async function (dispatch) {
    try {
      ...
      // Salva i dati dell'utente nel sessionStorage dopo l'aggiornamento della password
      if (data.user) {
        sessionStorage.removeItem("user");
        sessionStorage.setItem("user", JSON.stringify(data.user));
      }
...
export const resetPassword = (token, passwords) => async (dispatch) => {
  try {
    ...
    // Salva i dati dell'utente nel sessionStorage dopo il reset della password
    if (data.user) {
      sessionStorage.setItem("user", JSON.stringify(data.user));
    }

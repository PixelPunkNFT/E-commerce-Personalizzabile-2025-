...
const { isAuthentictedUser, authorizeRoles,checkSubscriptionStatus } = require("../middleWare/auth");
...
router.route("/admin/user/:id").get(isAuthentictedUser , authorizeRoles("admin") , getSingleUser).put(isAuthentictedUser , authorizeRoles("admin") ,checkSubscriptionStatus, updateUserRole).delete(isAuthentictedUser , authorizeRoles("admin") ,checkSubscriptionStatus, deleteUser)

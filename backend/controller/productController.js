...
exports.createProduct = asyncWrapper(async (req, res, next) => {
  ...
  // Parse sizeStock from string to array if it exists
  if (req.body.sizeStock) {
    try {
      let sizeStockData = JSON.parse(req.body.sizeStock);
      // Verifica che sizeStockData sia un array
      if (!Array.isArray(sizeStockData)) {
        return res.status(400).json({
          success: false,
          message: "Il formato delle taglie deve essere un array"
        });
      }
      // Verifica che ogni elemento abbia size e quantity
      for (let item of sizeStockData) {
        if (!item.size || !mongoose.Types.ObjectId.isValid(item.size)) {
          return res.status(400).json({
            success: false,
            message: "ID taglia non valido"
          });
        }
        if (typeof item.quantity !== 'number' && typeof parseInt(item.quantity) !== 'number') {
          return res.status(400).json({
            success: false,
      ...
      req.body.sizeStock = sizeStockData;
    } catch (error) {
      return res.status(400).json({
        success: false,
        message: "Errore nel parsing delle taglie",
        error: error.message
      });
    }
  }
  ...
  try {
    if (!req.body.name || !req.body.description || !req.body.price || !req.body.category || !req.body.info) {
      return res.status(400).json({
        success: false,
    ...
    if (!req.body.sizeStock || !Array.isArray(req.body.sizeStock) || req.body.sizeStock.length === 0) {
      return res.status(400).json({
        success: false,
        message: "Seleziona almeno una taglia con quantità"
      });
    }
...
exports.getAllProducts = asyncWrapper(async (req, res) => {
  ...
  const apiFeature = new ApiFeatures(ProductModel.find().populate('sizeStock.size'), req.query)
  ...
  products = await apiFeature.query.clone(); // Retrieve the paginated 
  products
...
exports.getAllProductsAdmin = asyncWrapper(async (req, res) => {
  const products = await ProductModel.find().populate('sizeStock.size');
...
exports.getProductDetails = asyncWrapper(async (req, res, next) => {
  ...
  const Product = await ProductModel.findById(id).populate('sizeStock.size');
  if (!Product) {
    return next(new ErrorHandler("Product not found", 404));
  }
  res.status(201).json({
